...
const ModulesScreen: React.FC<{ courseId?: number | string }> = ({
  ...
  const goToLessonsForModule = (moduleId: number) => {
    ...
    try {
      navigation.navigate("Lessons" as any, {
        ...
        try {
          ...
          if (
            ...
            (current.name === "Modules" &&
              (!cid || String(current.params?.courseId) === String(cid)))
          ) {
            clearWebRoute();
          } else {
          }
  ...
  const loadModules = async (courseId?: number) => {
    try {
      ...
      const cursoIds = Array.from(
        new Set(mods.map((m: any) => m.id_curso).filter(Boolean)),
      );
      if (cursoIds.length > 0) {
        ...
        if (!cursosRes.error && Array.isArray(cursosRes.data)) {
          ...
          (cursosRes.data || []).forEach((c: any) => {
            map[c.id_curso] = c.titulo;
          });
          setCoursesMap(map);
        }
      }
      ...
      <View style={styles.infoRow}>
        ...
        {item.obligatorio ? (
          <View style={[styles.infoTag, styles.requiredTag]}>
            ...
            <Text
              style={[styles.infoText, { color: "#D97706", fontWeight: "600" }]}
            >
              Obligatorio
            </Text>
          </View>
  ...
  return (
    ...
    >
          ...
          <View style={styles.warningContainer}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.warningText}>
                Para crear módulos o subir materiales, abre esta pantalla desde
                la página de un curso (Admin → Cursos → Módulos) o selecciona un
                curso primero.
              </Text>
            </View>
          </View>
      ...
      <ModuleFormModal
        ...
        courseId={parsedCourseId ? String(parsedCourseId) : ""}
