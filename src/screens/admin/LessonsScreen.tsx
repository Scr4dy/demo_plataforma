...
const LessonsScreen: React.FC<{ courseId?: number | string; moduleId?: number | string }> = ({ courseId: propCourseId, moduleId: propModuleId }) => {
  ...
  const parsedModuleId = rawMid == null ? undefined : (typeof rawMid === 'string' ? parseInt(rawMid, 10) : rawMid);
  ...
  useEffect(() => {
    ...
    const rawMid = route?.params?.moduleId ?? propModuleId;
    const mid = rawMid == null ? undefined : (typeof rawMid === 'string' ? parseInt(rawMid, 10) : rawMid);
    loadLessons(cid, mid);

    if (Platform.OS === 'web') {
      try { const { goToWebRoute } = require('../../utils/webNav'); if (cid) goToWebRoute('Lessons', { courseId: cid, moduleId: mid }); else goToWebRoute('Lessons'); } catch (e) {  }
    }
    ...
    return () => {
      if (Platform.OS === 'web') {
        try {
          ...
          if (!current || (current.name === 'Lessons' && ((current.params?.courseId == null && !cid) || (String(current.params?.courseId) === String(cid)) && (current.params?.moduleId == null || String(current.params?.moduleId) === String(mid))))) {
            clearWebRoute();
          } else {
          }
        } catch (e) {  }
  ...
  const loadLessons = async (courseId?: number, moduleId?: number) => {
    try {
      setLoading(true);
      let query = supabase.from('contenidos').select('*').is('deleted_at', null).order('orden', { ascending: true });
      if (courseId != null && !Number.isNaN(Number(courseId))) query = query.eq('id_curso', Number(courseId));
      if (moduleId != null && !Number.isNaN(Number(moduleId))) query = query.eq('id_modulo', Number(moduleId));
      const res: any = await query;
      if (res.error) throw res.error;
      const items = res.data || [];
      setLessons(items);

      const moduleIds = Array.from(new Set(items.map((i: any) => i.id_modulo).filter(Boolean)));
      if (moduleIds.length > 0) {
        const modsRes: any = await supabase.from('modulos').select('id_modulo, titulo').in('id_modulo', moduleIds);
        if (!modsRes.error && Array.isArray(modsRes.data)) {
          const map: Record<number, string> = {};
          (modsRes.data || []).forEach((m: any) => { map[m.id_modulo] = m.titulo; });
          setModulesMap(map);
        }
      }
    } catch (err) {
  ...
  const handleEdit = (id: number) => { setSelectedId(id); setShowFormModal(true); };
    ...
    Alert.alert('Confirmar eliminación', `¿Eliminar lección "${titulo}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar', style: 'destructive', onPress: async () => {
          try {
            const { error } = await supabase.from('contenidos').update({ deleted_at: new Date().toISOString() }).eq('id_contenido', id);
  ...
  const onSuccess = async () => { setShowFormModal(false); setSelectedId(null); await loadLessons(parsedCourseId, parsedModuleId); };

  React.useEffect(() => {
    
    setHeader({
      title: 'Lecciones',
      subtitle: (parsedModuleId ? modulesMap[parsedModuleId] : undefined) ?? undefined,
      showBack: true,
      manual: true,
      owner: 'Lessons',
  ...
  }, [setHeader, navigation, parsedModuleId, modulesMap, colors.primary, theme.colors.card]);
    ...
    <View style={[styles.card, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
      ...
      <View style={styles.infoRow}>
        <View style={styles.infoTag}>
          ...
          <Text style={[styles.infoText, { color: theme.colors.textSecondary }]}>
            {modulesMap[item.id_modulo] || (item.id_modulo ? `Módulo ${item.id_modulo}` : 'Sin módulo')}
          </Text>
        </View>
        {item.tipo && (
          <View style={styles.infoTag}>
      ...
      <View style={styles.cardActions}>
        ...
        <IconActionButton
          ...
          onPress={() => {
            if (Platform.OS === 'web') {
              try { const { goToWebRoute } = require('../../utils/webNav'); goToWebRoute('QuizManagement', { contentId: String(item.id_contenido), contentTitle: String(item.titulo || 'Quiz') }); return; } catch (e) {  }
            }

            
            try { navigation.navigate('QuizManagement' as any, { contentId: String(item.id_contenido), contentTitle: String(item.titulo || 'Quiz') }); } catch (e) {  }
          }}
          style={[styles.actionButton, Platform.OS !== 'web' && styles.actionButtonMobile]}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.content}>
        ...
        {Platform.OS !== 'web' ? (
          ...
          (isAdmin || isInstructor) && parsedCourseId ? (
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', marginBottom: 8 }}>
              <ThemedButton variant="primary" onPress={handleCreate} style={{ paddingHorizontal: 12 }} accessibilityLabel="Crear lección">Crear lección</ThemedButton>
            </View>
          ) : null
        )}
        ...
        {}
        {!parsedCourseId && (
          <View style={{ marginBottom: 12, padding: 12, borderRadius: 8, backgroundColor: '#FFF4E5', borderWidth: 1, borderColor: '#FFDAB9' }}>
            <Text style={{ fontWeight: '700', color: '#8A6D3B', marginBottom: 6 }}>Acciones no disponibles</Text>
            <Text style={{ color: '#8A6D3B' }}>
              Para crear lecciones o subir materiales, abre esta pantalla desde la página del curso (Admin → Cursos → Lecciones) o selecciona un módulo dentro de un curso.
            </Text>
          </View>
        )}
        ...
        {loading ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Text style={{ marginTop: 12, color: theme.colors.textSecondary }}>Cargando lecciones...</Text>
          </View>
        ) : (
          <FlatList
            data={lessons}
            ...
            ListEmptyComponent={
              <View style={{ alignItems: 'center', justifyContent: 'center', padding: 40, opacity: 0.6 }}>
                ...
                <Text style={{ marginTop: 12, fontSize: 16, color: theme.colors.textSecondary, textAlign: 'center' }}>
                  {parsedModuleId
                    ? 'Este módulo no tiene lecciones aún.'
                    : (parsedCourseId ? 'No hay lecciones en este curso.' : 'Selecciona un curso.')}
                </Text>
              </View>
            }
          />
        ...
        <Modal visible={showMaterialsModal} transparent animationType="slide" onRequestClose={() => setShowMaterialsModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' }}>
            <View style={{ width: '95%', maxWidth: 980, borderRadius: 12, overflow: 'hidden' }}>
              <CourseMaterialsPanel courseId={Number(parsedCourseId)} allowUpload={true} allowDelete={isAdmin} initialModuleId={materialModuleId ?? undefined} initialContenidoId={materialContenidoId ?? undefined} hideTargetSelection={materialContenidoId !== null} showFileList={false} onClose={() => setShowMaterialsModal(false)} onUploadDone={async () => { setShowMaterialsModal(false); await loadLessons(parsedCourseId, parsedModuleId); }} />
            </View>
          </View>
        </Modal>
      )}

      <LessonFormModal visible={showFormModal} courseId={parsedCourseId ? String(parsedCourseId) : (selectedId ? String(lessons.find(m => m.id_contenido === selectedId)?.id_curso) : (route?.params?.courseId ? String(route.params.courseId) : ''))} lessonId={selectedId || undefined} initialModuleId={parsedModuleId} onClose={() => { setShowFormModal(false); setSelectedId(null); }} onSuccess={onSuccess} />
    </View>
  );
};
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  content: { paddingHorizontal: 16, paddingTop: Platform.OS === 'web' ? 16 : 4, flex: 1 },
