...
export default function LessonDetailScreen(props?: { courseId?: string; moduleId?: string; content?: any; moduleTitle?: string; contentTitle?: string }) {
  ...
  const { courseId, moduleId, content, moduleTitle, contentTitle } = mergedParams || {};
  ...
  const lastHeaderRef = useRef<{ title?: string; subtitle?: string; hidden?: boolean }>({});
  React.useEffect(() => {
  ...
  const loadLessonData = async () => {
    try {
      ...
      if (!content && !courseId && !moduleId) {
        ...
        setLoading(false);
        if (Platform.OS !== 'web') showAlert('Error', 'No se pudo cargar la lección');
        return;
      }
      ...
      if (content && typeof content === 'object') {
        ...
        const contentId = typeof content === 'string' ? parseInt(content) : content;
  ...
  const loadProgress = async (contentId: number) => {
    try {
      ...
      if (data) {
        ...
        try { emit('progress:updated', { courseId }); } catch (e) {  }
      }
    } catch (error) {
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      const progressData = {
        ...
        fecha_completado: isCompletedRef.current && !progresoRef.current?.fecha_completado ? new Date().toISOString() : progresoRef.current?.fecha_completado,
        fecha_inicio: progresoRef.current?.fecha_inicio || new Date().toISOString(),
      };
      ...
      if (contenidoRef.current.id_curso) {
        try {
          await categoryService.syncCourseProgress(String(userData.id_usuario), String(contenidoRef.current.id_curso));
        } catch (syncErr) {
  ...
  const handleGoBack = async () => {
    ...
    const feedbackParams = isCompletedRef.current ? { showFeedback: true, courseId } : {};

    try {
      if (Platform.OS === 'web') {
        const { clearWebRoute, goToWebTab, goToWebRoute } = require('../../utils/webNav');
        const canGoBack = (typeof navigation?.canGoBack === 'function') ? navigation.canGoBack() : false;

        try { setHeader && setHeader(null); } catch (_) {  }
  ...
  if (loading) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background, zIndex: 0 }]} edges={isMobile ? ['left', 'right', 'bottom'] : ['bottom']}>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background, zIndex: 0 }]} edges={isMobile ? ['left', 'right', 'bottom'] : ['bottom']}>
        <View style={styles.errorContainer}>
          ...
          <Text style={[styles.errorText, { color: colors.text }]}>No se pudo cargar la lección</Text>

          <TouchableOpacity style={[styles.button, { backgroundColor: colors.primary }]} onPress={handleGoBack}>
            <Text style={[styles.buttonText, { color: theme.colors.card }]}>Volver</Text>
          </TouchableOpacity>

          {process.env.NODE_ENV !== 'production' && (
            <View style={{ marginTop: 14, padding: 12, backgroundColor: theme.colors.card, borderRadius: 8 }}>
              <Text style={{ color: colors.textSecondary, fontSize: 12 }}>
                Debug: params recibidos: {JSON.stringify(routeParamsDebug, null, 2)}
              </Text>
              <Text style={{ color: colors.textSecondary, fontSize: 12, marginTop: 8 }}>Sugerencia: Ve a Dashboard → Curso → Lección y haz click en la lección para que los parámetros se transmitan correctamente.</Text>
            </View>
          )}
  ...
  return (
    <SafeAreaView edges={['left', 'right', 'bottom']} style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.metadataCard, { backgroundColor: colors.card }]}>
          <View style={styles.metadataRow}>
            ...
            <View
              style={[styles.completeButtonInline, isCompleted && styles.completeButtonActive]}
            >
              <Ionicons
              ...
              />
              <Text style={[styles.metadataText, { color: isCompleted ? colors.success : colors.textSecondary }]}>
      ...
      {}
      {contenido?.tipo !== 'evaluacion' && (
        <View style={[styles.footer, { backgroundColor: colors.card, borderTopColor: theme.colors.border }]}>
          <TouchableOpacity
            style={[
              styles.footerButton,
              {
                backgroundColor: isCompleted ? '#4CAF50' : (canMarkComplete ? colors.primary : '#999'),
                opacity: 1
              }
            ]}
