...
export default function CertificatesScreen() {
  ...
  const loadCertificates = async () => {
    try {
      ...
      if (canCheckLimits) {
        tasks.push(
          supabase.rpc("obtener_cursos_elegibles_notificacion", {
      ...
      if (canCheckLimits && results.length >= 3) {
        ...
        if (elegiblesRes?.error) {
          ...
          const elegibles = elegiblesData.filter(
            (c: any) => c.puede_notificar === true,
          );
          const enviados = elegiblesData.filter(
            (c: any) => c.certificado_fisico_enviado === true,
          );
  ...
  const handleShareCertificate = async (certificate: Certificate) => {
    ...
    try {
      ...
      if (
        ...
        showAlert(
          "Info",
          "Tu navegador no permite copiar automáticamente. Usa Ctrl+C para copiar.",
        );
      } else {
        await Share.share({
          message,
    ...
    try {
      showConfirm(
        ...
        async () => {
          ...
          try {
            const res = await certificateService.downloadCertificate(
              certificate.id,
            );
            const url = res?.url;
              ...
              showConfirm(
                ...
                () => {
                  if (
                    ...
                    showAlert(
                      "Info",
                      "Tu navegador no permite copiar automáticamente. Selecciona y copia la URL manualmente.",
                    );
                  }
                },
                "Copiar URL",
              ...
              try {
                ...
                try {
                  ...
                  if (supported) {
                    ...
                    showAlert(
                      "Error",
                      "No se pudo descargar ni abrir el archivo.",
                    );
                  }
                } catch (e) {
                  showAlert(
                    "Error",
                    "Error al intentar descargar el certificado.",
                  );
                }
              }
            }
    ...
    return (
      ...
      >
        ...
        <Text
          style={[
            styles.loadingText,
            { color: theme.colors.text, fontSize: getFontSize(16) },
          ]}
        >
          {getLoadingMessage("certificates")}
        </Text>
      </View>
    );
  }
  ...
  const stats = {
    ...
    pending: certificates.filter(
      (c) => c.estado === "PENDIENTE" || (c.estado as any) === "POR_GENERAR",
    ).length,
  };
  ...
  const handleRequestSingleCertificate = async (item: any) => {
    try {
      ...
      if (result.success) {
        showAlert(
          "Solicitud enviada",
          `Se ha notificado a la administración para procesar tu certificado de "${courseTitle}".`,
          async () => {},
        );
        setWaitMinutes(60);
        await loadCertificates();
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
          ...
          >
            <View style={styles.statItemWeb}>
              <Text
                style={[
                  styles.statValueWeb,
                  { color: colors.primary, fontSize: getFontSize(28) },
                ]}
              >
                {stats.total}
              </Text>
            ...
            <View style={styles.statItemWeb}>
              <Text
                style={[
                  styles.statValueWeb,
                  { color: "#4CAF50", fontSize: getFontSize(28) },
                ]}
              >
                {stats.active}
              </Text>
            ...
            <View style={styles.statItemWeb}>
              <Text
                style={[
                  styles.statValueWeb,
                  { color: "#FF6B6B", fontSize: getFontSize(28) },
                ]}
              >
                {stats.expired}
              </Text>
        ...
        <View style={styles.certificatesList}>
          {(() => {
            const realCertCourseIds = new Set(
              certificates.map((c) => c.cursoId),
            );
              ...
              return (
                <View style={styles.emptyState}>
                  ...
                  <Text
                    style={[
                      styles.emptyTitle,
                      { color: theme.colors.text, fontSize: getFontSize(20) },
                    ]}
                  >
                    Sin certificados
                  </Text>
                  ...
                  {shippedCoursesCount > 0 && (
                    ...
                    >
                      ...
                      >
                        Todos tus certificados ({shippedCoursesCount}) ya fueron
            ...
            return (
              <>
                    ...
                    >
                          ...
                          >
                            <Ionicons
                              name={
                                isVirtual
                                  ? "time-outline"
                                  : (getStatusIcon(item.estado) as any)
                              }
                              size={14}
                        ...
                        <View style={styles.certificateActions}>
                          {!isVirtual && (
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                handleShareCertificate(item as any)
                              }
                          ...
                          {item.urlDescarga ? (
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                handleDownloadCertificate(item as any)
                              }
                              ...
                              return (
                                <TouchableOpacity
                                  ...
                                  onPress={() =>
                                    !isShipped &&
                                    waitMinutes === 0 &&
                                    handleRequestSingleCertificate(item)
                                  }
                                  disabled={
                      ...
                      {isVirtual && (
                        <View style={{ marginTop: 12 }}>
                          ...
                          >
                            Curso completado. Solicita tu certificado para que
                            la administración lo genere.
                          </Text>
                        </View>
                      )}
                    </View>
                ...
                {shippedCoursesCount > 0 && eligibleCourses.length === 0 && (
                  <View
                    style={{ marginTop: 10, alignItems: "center", padding: 15 }}
                  >
                    <Text
                      style={{
                        color: theme.dark ? "#4ade80" : "#16a34a",
                ...
                {requestsLeft === 0 && (
                  <View
                    style={{
                      ...
                      alignItems: "center",
                      padding: 10,
                      backgroundColor: theme.dark ? "#331111" : "#fff5f5",
                      borderRadius: 8,
                    }}
                  >
                    <Text
                ...
                {waitMinutes > 0 && requestsLeft > 0 && (
                  ...
                  >
                    ...
                    >
                      Por seguridad, debes esperar {waitMinutes} minutos para tu
                      siguiente solicitud.
                    </Text>
                  </View>
                )}
