...
export default function CourseDetailScreen(props: { courseId?: string; initialModuleId?: string } = {}) {
  ...
  const [evaluationMode, setEvaluationMode] = useState<'all' | 'instructor' | 'content'>('all');
  ...
  }, [params?.showFeedback, params?.forceFeedback, hasEvaluated, loading, courseProgress, hasCertificate, course]);
  ...
  const checkEvaluationStatus = async () => {
    try {
      ...
      const instructorEval = await evaluacionesService.getEvaluacionInstructor(Number(courseId), userId);
      const contentEval = await evaluacionesService.getEvaluacionContenido(Number(courseId), userId);
    ...
    setHeader({
      ...
      right: (
        <View style={{ flexDirection: 'row', gap: 8 }}>
          {canEvaluate && !hasEvaluated && !isCourseInstructor && !isInstructorDemo && (
            <TouchableOpacity onPress={() => setShowFeedbackModal(true)} accessibilityLabel="Evaluar curso" style={{ padding: 8 }}>
  ...
  const loadCourseData = async () => {
    ...
    try {
      ...
      const userIdNum = Number(state.user?.id_usuario || (state.user as any)?.id);

      if (resolvedIsEnrolled && !isNaN(userIdNum) && userIdNum > 0) {
        
        Promise.all([
          supabase.rpc('verificar_limite_solicitudes_diarias', { p_id_usuario: userIdNum, p_limite: 2 }),
          ...
          if (myCert) {
            :', { isShipped: myCert.fisicoEnviado });
      ...
      if (resolvedIsEnrolled && state.user?.id_usuario) {
        ...
        categoryService.syncCourseProgress(String(state.user.id_usuario), String(courseId))
          .then(syncedProgress => {
  ...
  const handleConfirmEnroll = async () => {
    try {
      ...
      if (result?.alreadyEnrolled) {
        showAlert('Ya estás inscrito', 'Ya tienes acceso completo a este curso', () => loadCourseData());
      } else if (result) {
        showAlert('Inscripción', 'Te has inscrito correctamente', () => loadCourseData());
      } else {
  ...
  const handleRequestCertificate = async () => {
    ...
    try {
      ...
      const userName = `${state.user.nombre || ''} ${state.user.apellidoPaterno || ''} ${state.user.apellidoMaterno || ''}`.trim() || userEmail || 'Usuario';

      

      const result = await certificateService.requestCertificatesForCompleted([courseIdNum], userId, userEmail, userName);

      if (result.success) {
        showAlert('Solicitud enviada', `Se ha notificado a la administración para procesar tu certificado de "${course.titulo || 'este curso'}".`);

        
        setWaitMinutes(60);

        
        const { data: limiteData } = await supabase.rpc('verificar_limite_solicitudes_diarias', {
          p_id_usuario: userId,
          p_limite: 2
        });
        ...
        if (isLimit) {
          ...
          setCertificate((prev: any) => prev ? { ...prev, fisicoEnviado: true } : { fisicoEnviado: true } as any);
        }
      }
    } catch (err) {
  ...
  const openContent = async (content: any, module?: Module) => {
    const userIsEnrolled = Boolean(enrollmentRecord || course?.inscrito || preview);
    if (!userIsEnrolled && !module?.isPreview) {
      showAlert('Contenido Bloqueado', 'Debes inscribirte para acceder a este contenido');
      return;
    }
              ...
              <View style={[styles.metaContainer, { marginTop: 6, gap: 12 }]}>
                ...
                {((enrollmentRecord || course?.inscrito || preview) && (courseProgress >= 0)) ? (
                  <View style={styles.metaItem}>
                    ...
                    <Text style={[styles.metaText, { color: theme.dark ? '#999' : '#666' }]}>{String(courseProgress)}% completado</Text>
                  </View>
                ) : null}
                <View style={styles.metaItem}>
                  <Ionicons name="layers-outline" size={16} color={theme.dark ? '#999' : '#666'} />
                  <Text style={[styles.metaText, { color: theme.dark ? '#999' : '#666' }]}>{String(modules.length)} módulos</Text>
                </View>
              </View>
            ...
            if (courseProgress === 100) {
              return (
                <View style={{ gap: 12 }}>
                      ...
                      <>
                        <TouchableOpacity
                          style={[
                            styles.continueButton,
                            {
                              backgroundColor: hasUrl ? colors.primary : (isShipped ? (theme.dark ? '#444' : '#eee') : (requestsLeft > 0 && waitMinutes === 0 ? colors.primary : '#999')),
                              opacity: (hasUrl || isShipped) ? 1 : (requestsLeft > 0 && waitMinutes === 0 ? 1 : 0.6),
                              width: '100%'
                            }
                          ]}
                          disabled={(!hasUrl && !isShipped && (requestsLeft === 0 || waitMinutes > 0)) || isShipped || requestingCertificate}
                          onPress={async () => {
                            if (hasUrl) {
                              ...
                              if (Platform.OS === 'web') {
                                require('../../utils/webNav').goToWebRoute('Certificates');
                              } else {
                        ...
                        >
                          <Ionicons
                            name={hasUrl ? "document-text-outline" : (isShipped ? "checkmark-circle-outline" : "mail-outline")}
                            size={20}
                            color={isShipped ? (theme.dark ? '#888' : '#aaa') : "#fff"}
                          />
                          <Text style={[
                            styles.continueButtonText,
                            isShipped && { color: theme.dark ? '#888' : '#aaa' }
                          ]}>
                            {hasUrl ? 'Descargar certificado' : (isShipped ? 'Enviado por correo' : (waitMinutes > 0 ? `Espera ${waitMinutes} min` : (requestsLeft > 0 ? (requestingCertificate ? 'Solicitando...' : 'Solicitar certificado') : 'Límite diario alcanzado')))}
                          </Text>
                        </TouchableOpacity>

                        {requestsLeft < 2 && requestsLeft > 0 && !hasUrl && !isShipped && (
                          <Text style={{ textAlign: 'center', fontSize: 12, color: theme.dark ? '#999' : '#666', marginTop: -4 }}>
                            {waitMinutes > 0
                  ...
                  {}
                  {!isCourseInstructor && !isInstructorDemo && (!hasEvaluatedInstructor || !hasEvaluatedContent) && (
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                      {}
                      {!hasEvaluatedInstructor && (
                        ...
                        >
                          ...
                          <Text style={[styles.continueButtonText, { color: '#000', fontSize: 13, textAlign: 'center', fontWeight: '600' }]}>
                            Evaluar Instructor
                          </Text>
                        </TouchableOpacity>
                      )}
                      ...
                      {!hasEvaluatedContent && (
                        ...
                        >
                          ...
                          <Text style={[styles.continueButtonText, { color: '#fff', fontSize: 13, textAlign: 'center', fontWeight: '600' }]}>
                            Evaluar Contenido
                          </Text>
                        </TouchableOpacity>
                      )}
        ...
        {isEnrolled ? (
          <View style={[styles.progressSection, { backgroundColor: theme.colors.card }]}>
            <View style={styles.progressHeader}>
              ...
              <Text style={[styles.progressPercentage, { color: colors.primary }]}>{String(courseProgress)}%</Text>
            </View>
            ...
            <Text style={[styles.progressText, { color: theme.dark ? '#999' : '#666' }]}>
              {String(modules.filter(m => m.completed).length)} de {String(modules.length)} módulos completados
            </Text>
          </View>
        ) : null}
        ...
        <View style={styles.modulesSection}>
          ...
          {modules.length === 0 ? (
            <View style={{
              ...
              <Ionicons name="school-outline" size={48} color={theme.dark ? '#555' : '#ccc'} style={{ marginBottom: 12 }} />
              <Text style={{
