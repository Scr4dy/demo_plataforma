...
const AllCoursesScreen: React.FC<AllCoursesProps> = ({ adminMode = false }) => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "active" | "inactive"
  >("all");
  const [sortBy, setSortBy] = useState<"titulo" | "fecha_creacion" | "activo">(
    "titulo",
  );
  const [modalVisible, setModalVisible] = useState(false);
  const [editingCourse, setEditingCourse] = useState<string | undefined>(
    undefined,
  );
  ...
  const [selectedCourseMaterialsId, setSelectedCourseMaterialsId] = useState<
    string | null
  >(null);
  ...
  const filteredAdmin = useMemo(() => {
    ...
    list.sort((a: any, b: any) => {
      ...
      if (sortBy === "fecha_creacion")
        return (
          (new Date(b.fecha_creacion).getTime() || 0) -
          (new Date(a.fecha_creacion).getTime() || 0)
        );
      if (sortBy === "activo")
  ...
  useEffect(() => {
        ...
        right: (
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            >
              <Text
                style={{
                  color: showAdmin ? colors.primary : theme.colors.text,
                  fontWeight: "600",
                }}
              >
                {showAdmin ? "Modo Admin" : "Ver público"}
              </Text>
            </TouchableOpacity>
          </View>
        ),
    ...
    return () => {
      try {
        if (
          header &&
          (header.owner === "AllCourses" || header.owner === "AllCoursesAdmin")
        )
          setHeader(null);
      } catch (e) {}
    };
  ...
  const numColumns =
    isWeb && width >= 768
      ? Math.max(
          ...
          Math.min(
            MAX_COLUMNS,
            Math.floor((width - 40) / DESIRED_CARD_MIN_WIDTH),
          ),
        )
      : 1;
  ...
  const loadCourses = useCallback(async (useLoadingState = true) => {
    ...
    try {
      ...
      const enriched = (data || []).map((c: any) => {
        ...
        const catById = cats.find(
          (ct) =>
            String(ct.id) ===
            String(c.categoria_id || c.categoriaId || c.categoria_id_hash),
        );
        if (catById) return { ...c, categorias: catById };
        const catByName = cats.find(
          (ct) =>
            String(ct.nombre).toLowerCase() ===
            String(c.categoria || c.categorias?.nombre || "").toLowerCase(),
        );
        ...
        if (instructorIds.length > 0) {
          try {
            const { data: usersByIdUsuario } = await supabase
              ...
              .in("id_usuario", instructorIds as any);
            usersByIdUsuario?.forEach((u: any) => {
              if (u?.id_usuario)
                instructorMap[String(u.id_usuario)] = u.nombre || "";
            });
  ...
  const loadAdminCourses = useCallback(async () => {
    ...
    try {
      ...
      const enriched = (list || []).map((c: any) => {
        ...
        let categoriaObj = cats.find(
          (ct) =>
            String(ct.nombre).toLowerCase() ===
            String(categoriaNombre).toLowerCase(),
        );
        if (!categoriaObj && c.categoria_id) {
          categoriaObj = cats.find(
            (ct) => String(ct.id) === String(c.categoria_id),
          );
        }
  ...
  const renderItem = ({ item }: any) => {
    const containerStyle: any = isMobile
      ? { width: "100%", paddingHorizontal: 0 }
      : {
          width: Math.min(
            DESIRED_CARD_MIN_WIDTH,
            Math.max(
              260,
              Math.floor(
                (Math.min(1400, numColumns * DESIRED_CARD_MIN_WIDTH + 40) -
                  (numColumns - 1) * cardGap -
                  40) /
                  numColumns,
              ),
            ),
          ),
          paddingHorizontal: Math.floor(cardGap / 2),
        };
      ...
      <View style={containerStyle}>
        ...
        >
          ...
          <Text
            style={[
              styles.actionText,
              { color: colors.primary, fontSize: getFontSize(14) },
            ]}
          >
            Ver
          </Text>
    ...
    >
      <FlatList
        ...
        keyExtractor={(item, index) =>
          String(item.id || item.id_curso || item._id || String(index))
        }
        ...
        contentContainerStyle={[
          ...
          isWeb &&
            isDesktop && {
              maxWidth: Math.min(
                1400,
                numColumns * DESIRED_CARD_MIN_WIDTH + 40,
              ),
              alignSelf: "center",
              alignItems: "center",
            },
