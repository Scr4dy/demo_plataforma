...
import { categoryService, Categoria, Curso } from '../../services/categoryService';
...
const CategoriesScreen = () => {
  ...
  const usingBackend = !!((AppConfig.supabase?.url && AppConfig.supabase?.anonKey) || (SUPABASE_CONFIG?.URL && SUPABASE_CONFIG?.ANON_KEY));

  const { verificarPuedeEvaluar } = useEvaluaciones();

  const [showEvaluationModal, setShowEvaluationModal] = useState(false);
  const [evaluationCourseId, setEvaluationCourseId] = useState<number | null>(null);
  const [evaluationCourseTitle, setEvaluationCourseTitle] = useState<string>('');

  const cargarDatos = React.useCallback(async (useLoadingState = true) => {
    try {
      ...
      .length, 'courses:', (cursosData || []).length);
       => ({
        titulo: c.titulo,
        inscrito: c.inscrito,
        tipo_inscrito: typeof c.inscrito,
        progreso: c.progreso
      })));

      
      if ((categoriasData || []).length === 0 && (cursosData || []).length === 0) {
      ...
      if (categoriasData && categoriasData.length > 0) {
        ...
        const enrichedCursos = (cursosData || []).map((c: any) => {
          ...
          if (!cat && c.categoria) {
            ...
            cat = categoriasData.find(ct => String(ct.nombre).trim().toLowerCase() === catName);
          }
        ...
        (cursosData || []).forEach((c: any) => {
          ...
          if (!derivedMap.has(id)) {
            derivedMap.set(id, {
              id: id,
              nombre: name,
              descripcion: name !== 'Sin categoría' ? `Cursos de ${name}` : 'Cursos sin categoría asignada',
  ...
  const cursosFiltrados = React.useMemo(() => {
    ...
    const sel = categorias.find(ct => ct.id === categoriaSeleccionada) || null;

    
    if (!sel) {
      return cursos.filter(curso => String(curso.categoria_id) === String(categoriaSeleccionada) || String((curso as any).categoriaId) === String(categoriaSeleccionada));
    }

    
    if (String(sel.nombre).toLowerCase() === 'sin categoría') {
      return cursos.filter(curso => {
        const nombre = String(curso.categoria || curso.categorias?.nombre || '').trim();
        return nombre === '' || nombre.toLowerCase() === 'sin categoría' || !curso.categoria_id;
      });
    }
    ...
    return cursos.filter(curso => {
      ...
      if ((curso as any).id_categoria && String((curso as any).id_categoria) === String(sel.id)) return true;
      if (curso.categoria_id && String(curso.categoria_id) === String(sel.id)) return true;
      if ((curso as any).categoriaId && String((curso as any).categoriaId) === String(sel.id)) return true;

      
      const nombre = String(curso.categoria || curso.categorias?.nombre || '').trim().toLowerCase();
      return nombre === String(sel.nombre || '').trim().toLowerCase();
    });
  ...
  const inscribirEnCurso = async (cursoId: string | number) => {
    try {
      ...
      const inscripcion = await categoryService.inscribirEnCurso(state.user.id, cursoIdStr);

      
      setCursos(prevCursos => {
        const updated = prevCursos.map(curso => {
          if (String(curso.id) === cursoIdStr) {
            return { ...curso, inscrito: true, progreso: 0, inscripcionEstado: 'en_progreso' };
          }
          return curso;
        });
        return updated;
      ...
      Alert.alert('Error', `No se pudo completar la inscripción${error?.message ? ': ' + error.message : ''}`);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {!usingBackend && (
        <View style={{ padding: 8, backgroundColor: '#FFF3CD', borderRadius: 8, margin: 12 }}>
          <Text style={{ color: '#856404' }}>🔧 Modo de desarrollo: usando datos de ejemplo (mock).</Text>
        </View>
      )}

      {}
      {hasBackendError && usingBackend && (
        <View style={{ margin: 12, padding: 16, backgroundColor: '#FFF3CD', borderRadius: 12, borderLeftWidth: 4, borderLeftColor: '#FF6B6B' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
            <Ionicons name="warning" size={24} color="#856404" style={{ marginRight: 8 }} />
            <Text style={{ fontSize: 16, fontWeight: '700', color: '#856404', flex: 1 }}>
          ...
          <Text style={{ fontSize: 14, color: '#856404', marginBottom: 12, lineHeight: 20 }}>
            No se encontraron categorías ni cursos. Posibles causas:{'\n'}
            • Tabla "categorias" no existe o relación no configurada{'\n'}
            • Sin cursos activos (activo='true'){'\n'}
            • Problema de permisos o RLS en Supabase
          </Text>

          {showDiagnostics && (
            <View style={{ marginTop: 8, gap: 8 }}>
              <Text style={{ fontSize: 13, fontWeight: '600', color: '#856404', marginBottom: 4 }}>
              ...
              <View style={{
                ...
                <Text style={{ color: '#2E7D32', fontSize: 13, marginBottom: 6, fontWeight: '600' }}>
                  ✅ Adaptaciones automáticas:
                </Text>
                <Text style={{ color: '#2E7D32', fontSize: 12, lineHeight: 18 }}>
                  • Usando id_curso como PK{'\n'}
                  • Filtrando por activo='true'{'\n'}
                  • Generando categorías desde cursos.categoria{'\n'}
                  • Sin necesidad de modificar BD
                </Text>
              </View>
              ...
              >
                <Ionicons name="refresh" size={18} color={theme.colors.card} style={{ marginRight: 8 }} />
                <Text style={{ color: theme.colors.card, fontWeight: '600', fontSize: 14 }}>
                  Reintentar carga
                </Text>
              </TouchableOpacity>

              <Text style={{ fontSize: 11, color: colors.warning, marginTop: 8, fontStyle: 'italic' }}>
                💡 Si siguen sin aparecer, verifica que haya cursos con activo='true' en Supabase.
              </Text>
            </View>
          )}
        </View>
      ...
      {}
      <View style={[styles.categoriasContainer, { backgroundColor: theme.colors.card }]}>
        <ScrollView
          horizontal
        ...
        >
          <TouchableOpacity
            style={[
              ...
              { backgroundColor: categoriaSeleccionada ? theme.colors.card : colors.primary, borderColor: categoriaSeleccionada ? theme.colors.border : colors.primary }
            ]}
            onPress={() => setCategoriaSeleccionada(null)}
          >
            <Text style={[
              styles.categoriaChipText,
              { color: categoriaSeleccionada ? theme.colors.text : theme.colors.card, fontWeight: categoriaSeleccionada ? '500' : '700' }
            ]}>
              Todos
            </Text>
          </TouchableOpacity>
          ...
          {categorias.map((categoria) => (
            <TouchableOpacity
              ...
              style={[
                styles.categoriaChip,
                {
                  backgroundColor: categoriaSeleccionada === categoria.id ? (categoria.color || getColorCategoria(categoria.nombre)) : theme.colors.card,
                  borderColor: categoriaSeleccionada === categoria.id ? (categoria.color || getColorCategoria(categoria.nombre)) : theme.colors.border
                }
              ]}
              onPress={() => setCategoriaSeleccionada(categoria.id)}
            >
              <Ionicons
                name={(categoria.icono || getIconoCategoria(categoria.nombre)) as any}
                size={16}
                color={categoriaSeleccionada === categoria.id ? theme.colors.card : theme.colors.text}
                style={{ marginRight: 6 }}
              />
              <Text style={[
                styles.categoriaChipText,
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              <View style={styles.cursoHeader}>
                <View style={styles.cursoInfo}>
                  <View style={[
                    ...
                    { backgroundColor: item.categorias?.color || getColorCategoria(item.categorias?.nombre || ''), flexDirection: 'row', alignItems: 'center' }
                  ]}>
                    <Ionicons name={(item.categorias?.icono || 'school') as any} size={12} color="#fff" style={{ marginRight: 4 }} />
                    <Text style={styles.categoriaBadgeText}>
                      {item.categoria || item.categorias?.nombre || 'Sin categoría'}
                    </Text>
                  </View>
                  {!item.inscrito && item.es_publico && (
                    <View style={[styles.previewBadge, { backgroundColor: theme.dark ? hexToRgba('#ffffff', 0.06) : hexToRgba(colors.primary, 0.12) }]}>
              ...
              <Text style={[styles.cursoDescripcion, { color: theme.dark ? '#999' : '#666' }]} numberOfLines={2}>
              ...
              <View style={styles.cursoFooter}>
                <View style={styles.cursoMeta}>
                  {item.instructor && item.instructor.trim() !== '' && (
                    <Text style={styles.cursoInstructor}>Por: {item.instructor}</Text>
                  )}
                  <View style={styles.durationContainer}>
                    <Ionicons name="time-outline" size={12} color={theme.dark ? '#999' : '#666'} />
                ...
                {item.fecha_fin && (
                  <View style={[styles.expirationContainer, {
                    backgroundColor: isExpired(item.fecha_fin) && item.progreso !== 100
                      ? (theme.dark ? '#3a1515' : '#ffebee')
                      : isExpiringSoon(item.fecha_fin) && item.progreso !== 100
                        ? (theme.dark ? '#3a2f15' : '#fff8e1')
                        : (theme.dark ? 'rgba(59, 130, 246, 0.2)' : '#e0f2fe'), 
                    borderColor: isExpired(item.fecha_fin) && item.progreso !== 100
                      ? '#FF5252'
                      : isExpiringSoon(item.fecha_fin) && item.progreso !== 100
                        ? '#FFA000'
                        : (theme.dark ? '#1e40af' : '#7dd3fc'), 
                    borderWidth: 1,
                    paddingHorizontal: 8,
                    paddingVertical: 4,
                    borderRadius: 6,
                    ...
                    <Ionicons
                      name={isExpired(item.fecha_fin) && item.progreso !== 100 ? "alert-circle" : isExpiringSoon(item.fecha_fin) && item.progreso !== 100 ? "warning" : "calendar-outline"}
                      size={12}
                      color={isExpired(item.fecha_fin) && item.progreso !== 100 ? "#FF5252" : isExpiringSoon(item.fecha_fin) && item.progreso !== 100 ? "#FFA000" : theme.dark ? "#60a5fa" : "#0284c7"}
                    />
                    <Text style={[
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateText}>{getEmptyStateMessage('courses')}</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 4 }}>
              <Text style={styles.emptyStateSubtext}>
                Intenta explorar otras categorías o vuelve más tarde
              </Text>
              <Ionicons name="search-outline" size={14} color="#999" style={{ marginLeft: 4 }} />
            </View>
          </View>
        }
      />
      ...
      <CourseEvaluationModal
        ...
        empleadoId={Number((state.user as any)?.id_usuario || state.user?.id) || 0}
