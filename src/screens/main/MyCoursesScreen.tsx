...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, useWindowDimensions, Platform } from 'react-native';
    ...
    try {
      ...
      const initialCourses = (cursosData || []).map(curso => {
        ...
        const inscripcion = inscripciones.find(i =>
          String(i.id_curso) === String(curso.id_curso)
        );
      ...
      if (cursosData && targetUserId) {
        Promise.all(
          cursosData.map(curso =>
            categoryService.syncCourseProgress(String(targetUserId), String(curso.id_curso))
              .catch(err => )
          )
        ).then(() => {
  ...
  useEffect(() => {
    const applyHeader = () => {
      setHeader({
        ...
        showBack: true,
        onBack: () => { try { const { safeBack } = require('../../utils/navigationHelpers'); safeBack(navigation); } catch (e) {  } },
        manual: true,
      });
    };
    ...
    if (Platform.OS === 'web') {
      ...
      return () => { clearTimeout(t); if (header && header.manual && header.title === 'Mis Cursos') setHeader(null); };
    }
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (isBlocked) {
            ...
            if (Platform.OS === 'web') 
            setExpirationMessage(`Este curso venció el ${formatExpirationDate(item.fecha_fin)}. Ya no está disponible para realizar.`);
      ...
      >
        <View style={[
          ...
          { backgroundColor: (item.progreso || 0) >= 100 ? '#E8F5E9' : colors.primaryLight }
        ]}>
          <Text style={[
            styles.progressText,
        ...
        <Text style={[styles.courseCategory, { color: theme.dark ? '#999' : '#666' }]} numberOfLines={1}>
        ...
        <View style={styles.footer}>
          <View style={styles.footerItem}>
            ...
            <Text style={[styles.footerText, { color: theme.dark ? '#999' : '#666' }]}>
          ...
          {item.fecha_fin && (
            <View style={[styles.footerItem, {
              backgroundColor: isExpired(item.fecha_fin) && (item.progreso || 0) < 100
                ? (theme.dark ? '#3a1515' : '#ffebee')
                : (theme.dark ? 'rgba(59, 130, 246, 0.2)' : '#e0f2fe'),
              borderColor: isExpired(item.fecha_fin) && (item.progreso || 0) < 100
                ? '#FF5252'
              ...
              <Ionicons
                name={isExpired(item.fecha_fin) && (item.progreso || 0) < 100 ? "alert-circle" : "calendar-outline"}
                size={12}
                color={isExpired(item.fecha_fin) && (item.progreso || 0) < 100 ? "#FF5252" : theme.dark ? "#60a5fa" : "#0284c7"}
              />
              <Text style={[
                styles.footerText,
                {
                ...
                {isExpired(item.fecha_fin) ? 'Venció' : 'Vence'}: {formatExpirationDate(item.fecha_fin)}
              </Text>
            </View>
          )}
          ...
          {item.estado === 'completado' && (
            <View style={[styles.completedBadge, { backgroundColor: '#4CAF50' }]}>
              ...
              <Text style={[styles.completedText, { color: theme.colors.card }]}>Completado</Text>
            </View>
          )}
        </View>
      </TouchableOpacity>
  ...
  if (loading) {
    return (
      <View style={[styles.container, { backgroundColor: theme.colors.background, justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={[styles.loadingText, { color: theme.colors.text }]}>Cargando tus cursos...</Text>
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {!isMobile && (
        <InlineHeader
          ...
          showOnWeb={false}
          containerStyle={{ backgroundColor: theme.colors.card, borderBottomColor: theme.dark ? '#333' : '#e0e0e0' }}
        />
      )}
      <FlatList
        data={courses}
        ...
        contentContainerStyle={[
          ...
          isWeb && isDesktop && { maxWidth: 1400, alignSelf: 'center', width: '100%' }
        ]}
        columnWrapperStyle={numColumns > 1 ? { gap: isMobile ? 14 : isTablet ? 18 : 20 } : undefined}
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Ionicons name="book-outline" size={64} color={theme.dark ? '#555' : '#ccc'} />
            <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>No hay cursos inscritos</Text>
