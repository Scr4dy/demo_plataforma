...
import type {
  DBEvaluacionContenido,
  DBEvaluacionInstructor,
...
class EvaluacionesService {
  ...
  async getEvaluacionesContenidoByCurso(
    cursoId: number,
  ): Promise<DBEvaluacionContenido[]> {
    try {
      const { data, error } = await supabase
        .from("evaluaciones_contenido")
        .select("*")
  ...
  async getEvaluacionesInstructorByCurso(
    cursoId: number,
  ): Promise<DBEvaluacionInstructor[]> {
    try {
      const { data, error } = await supabase
        .from("evaluaciones_instructor")
        .select("*")
  ...
  async getPromedioContenido(
    ...
    try {
      ...
      const utilidadPromedio =
        evaluaciones.reduce((sum, e) => sum + e.utilidad, 0) /
        evaluaciones.length;
      const comprensionPromedio =
        evaluaciones.reduce((sum, e) => sum + e.comprension, 0) /
        evaluaciones.length;
