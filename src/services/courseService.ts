  ...
  async getMyCourses(options?: CacheOptions): Promise<Course[]> {
    try {
      const { data, error } = await supabase
        .from("cursos")
        .select(
          `*, usuarios:id_instructor (id_usuario, nombre, apellido_paterno, apellido_materno), categorias (nombre, color, icono)`,
        )
        .eq("activo", "true")
        .is("deleted_at", null)
        .order("titulo");
      ...
      return (data || []).map((c: any) => {
        ...
        return {
          ...
          progreso: 0,
          instructor:
            instructorName || c.instructor || c.instructor_nombre || "",
    ...
    try {
      const [
        ...
        supabase
          ...
          .select("id_certificado", { head: true, count: "exact" }),
        supabase
      ...
      const cursosPendientes = Math.max(
        0,
        totalCursos - cursosCompletados - cursosEnProgreso,
      );
      const certificaciones = Number(certResp.count || 0);
      const evaluacionesCompletadas = Number(evalResp.count || 0);
      const durations = (hoursResp.data || []).map((r: any) =>
        Number(r.duracion || 0),
      );
      const totalHoras = durations.length
  ...
  async getCourseDetail(
    ...
    try {
      ...
      const mapped: Course = {
        ...
        instructor:
          instructorName ||
          (c.instructor && c.instructor !== "Instructor" ? c.instructor : ""),
        categoria: c.categorias?.nombre || c.categoria || "Sin categoría",
        categorias: c.categorias,
        activo: c.activo,
        ...
        for (const uKey of userKeys) {
          try {
            ...
            return {
              cursoId: courseId,
              progreso: progress,
              leccionesCompletadas: Math.floor(progress / 10),
              totalLecciones: 10,
    ...
    try {
      ...
      try {
        ...
        const { data: byAuth, error: errorByAuth } = await supabase
          .from("usuarios")
          .select("id_usuario, id, auth_id, numero_empleado")
        ...
        if (!usuarioRecord) {
          const { data: byControl, error: errorByControl } = await supabase
            .from("usuarios")
            .select("id_usuario, id, auth_id, numero_control")
        ...
        if (usuarioRecord) {
          empleadoId =
            usuarioRecord.id_usuario || usuarioRecord.id || usuarioId;
        }
      } catch (e) {}
      ...
      const { data, error } = await insertWithUsuarioAndCursoFallback(
        "inscripciones",
        payload,
      );
      if (error) throw error;
