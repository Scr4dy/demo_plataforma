...
class UserService {
    ...
    if (!supabaseConfigured) {
      return [
        ...
        },
        {
          ...
          description:
            "Carlos Rodríguez se inscribió en Seguridad Industrial Avanzada",
        },
        {
          id: "3",
          type: "completion",
        ...
        },
        {
          ...
          description:
            "Roberto Jiménez obtuvo certificado en Gestión de Procesos",
        },
        {
          id: "5",
          type: "enrollment",
  ...
  async getInstructors(
    roles: string[] = ["Instructor", "Administrador"],
  ): Promise<Array<{ id: string; name: string; id_usuario?: string }>> {
    if (!supabaseConfigured) {
      return [];
    }
  ...
  async searchUsers(
    criteria: any,
  ): Promise<{ content: AdminUser[]; totalElements: number }> {
    if (!supabaseConfigured) {
