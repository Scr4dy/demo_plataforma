...
class AuthService {
    ...
    try {
      ...
      const authResponse: AuthResponse = {
        ...
        usuario: {
          ...
          nombreCompleto: profile ? `${profile.nombre} ${profile.apellido_paterno} ${profile.apellido_materno || ''}`.trim() : data.user.email?.split('@')[0] || 'Usuario',
          usuario: profile?.numero_control || data.user.email?.split('@')[0] || 'usuario',
          correoElectronico: profile?.correo || data.user.email || credentials.email,
          ...
          fechaActualizacion: profile?.ultimo_acceso || data.user.updated_at || data.user.created_at,
  ...
  async register(userData: RegisterRequest): Promise<{ mensaje: string; usuarioId: number }> {
    ...
    if (userData.telefono && userData.telefono.trim()) {
      ...
      if (!/^\+?[0-9]{10,15}$/.test(telefonoSanitizado)) {
        throw new Error('El teléfono debe contener entre 10 y 15 dígitos numéricos (ejemplo: 5551234567 o +525551234567)');
      }
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Email not confirmed')) {
          throw new Error('Correo no confirmado. Revisa tu bandeja de entrada.');
        }
      ...
      if (insertError) {
        ...
        if (insertError.code === '23505' && insertError.message?.includes('auth_id')) {
          ...
          return {
            mensaje: 'Usuario registrado exitosamente. Revisa tu email para confirmar tu cuenta.',
            usuarioId: 0 
          };
        }
        ...
        if (insertError.message.includes('Ya existe un usuario') ||
          insertError.message.includes('numero_control') ||
          insertError.message.includes('correo') ||
          insertError.message.includes('duplicate key') ||
          insertError.message.includes('unique constraint')) {
          throw new Error('Ya existe un usuario registrado con este correo o número de control. Por favor intenta iniciar sesión.');
        }

        throw new Error('Error al crear el registro del usuario: ' + insertError.message);
      }
  ...
  async refreshToken(): Promise<AuthResponse> {
    ...
    if (error || !data.session) {
      ...
      const isRefreshError = errorMsg.includes('refresh_token_not_found') ||
    ...
    return {
      ...
      usuario: {
        ...
        nombreCompleto: profile?.nombre_completo || data.user!.email?.split('@')[0] || 'Usuario',
        usuario: profile?.usuario || data.user!.email?.split('@')[0] || 'usuario',
  ...
  async getCurrentUser(): Promise<UserResponse | null> {
    ...
    return {
      ...
      nombreCompleto: profile?.nombre_completo || user.email?.split('@')[0] || 'Usuario',
