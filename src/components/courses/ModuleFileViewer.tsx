...
export default function ModuleFileViewer({
  visible,
  onClose,
  url,
  mime,
  ...
  const [embeddedViewerUrl, setEmbeddedViewerUrl] = useState<string | null>(
    null,
  );
  const [embeddedViewerLoading, setEmbeddedViewerLoading] =
  ...
  useEffect(() => {
    ...
    const isImage =
      ...
      !!filenameLower.match(/\.(jpg|jpeg|png|gif|webp|bmp)$/i);
    const isVideo = type.startsWith("video/");
    const isPdf = type === "application/pdf" || filenameLower.endsWith(".pdf");
    const isOffice =
      type.includes("officedocument") ||
      !!filenameLower.match(/\.(ppt|pptx|doc|docx|xls|xlsx)$/i);
  ...
  useEffect(() => {
    ...
    async function fetchText() {
      ...
      const isText =
        ...
        String(filename || "").match(/\.(json|txt|md)$/i);
      if (!isText) {
        setTextContent(null);
        setTextError(null);
        return;
      ...
      try {
        ...
        if (mounted)
          setTextError("No fue posible obtener el contenido de texto");
      } finally {
  ...
  useEffect(() => {
    ...
    async function fetchEmbeddedForPPT() {
      ...
      try {
        ...
        const proxyPath = origin
          ? `${origin}${proxyPathRelative}`
          : proxyPathRelative;
  ...
  const renderContent = () => {
    if (
      type.startsWith("image/") ||
      String(filename || "").match(/\.(jpg|jpeg|png|gif|webp|bmp)$/i)
    ) {
      if (isWeb) {
    ...
    if (
      ...
      String(filename || "").match(/\.(mp3|wav|ogg)$/i)
    ) {
    ...
    if (type.startsWith("video/")) {
      ...
      try {
        ...
        try {
          ...
          return (
            <Text style={{ color: colors.text }}>
              No se puede reproducir el video en este dispositivo.
            </Text>
          );
        }
      }
      ...
      if (
        ...
        try {
          const { WebView } = require("react-native-webview");
          const getPdfWebViewHtml = (zoom = 1) =>
            `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=${zoom}"></head><body style="margin:0;height:100%"><iframe src="${url}" style="border:none;width:100%;height:100%"></iframe></body></html>`;
          return (
            <WebView
              originWhitelist={["*"]}
              source={{ html: getPdfWebViewHtml() }}
              style={{ flex: 1 }}
            />
          );
        } catch (e) {
          return (
            <Text style={{ color: colors.text }}>
              No se puede mostrar el PDF en este dispositivo.
            </Text>
          );
        }
      }
      ...
      if (
        type.includes("officedocument") ||
        String(filename || "").match(/\.(ppt|pptx|doc|docx|xls|xlsx)$/i)
      ) {
        const ext =
          String(filename || "")
            .split(".")
        ...
        if (!allowRender) {
          return (
            <View style={{ padding: 24, alignItems: "center" }}>
              ...
              >
                Este archivo no se mostrará automáticamente para evitar
                descargas no deseadas.
              </Text>
              <TouchableOpacity
                onPress={() => setAllowRender(true)}
                style={{
                  padding: 12,
                  backgroundColor: colors.primary,
                  borderRadius: 8,
            ...
            if (pdfRenderFailed) {
              return (
                <View style={{ padding: 24, alignItems: "center" }}>
                  ...
                  >
                    No fue posible renderizar el documento. Puedes abrirlo en el
                    visor integrado sin descargar o intentar abrirlo
                    externamente.
                  </Text>
                  <TouchableOpacity
                    onPress={async () => {
                      if (!originalPath) {
                        Alert.alert("Error", "Ruta original desconocida.");
                        return;
                      }
                      try {
                        const ext =
                        ...
                        setConversionError(
                          "No fue posible abrir el visor integrado",
                        );
                      }
                    }}
                    style={{
                  ...
                  >
                    <Text style={{ color: "#fff", fontWeight: "600" }}>
                      Abrir en visor integrado (sin descargar)
                    </Text>
                  </TouchableOpacity>
          ...
          <View style={{ padding: 24, alignItems: "center" }}>
            {conversionLoading ? (
              ...
              <>
                ...
                >
                  Este archivo no puede previsualizarse directamente en este
                  dispositivo. Puedes intentar convertirlo a PDF o abrirlo en un
                  visor integrado.
                </Text>
                ...
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <ThemedButton
                    ...
                    onPress={async () => {
                      if (!originalPath) {
                        Alert.alert(
                          "Error",
                          "Ruta original desconocida. No se puede convertir.",
                        );
                        return;
                      }
                      ...
                      try {
                        const signedUrl =
                          await storageService.getCourseContentUrl(
                            originalPath,
                            3600,
                          );
                        const {
                        ...
                        if (!resp.ok) {
                          ...
                          setConversionError(
                            txt ||
                              "Error en conversión (posible que el servicio no esté habilitado)",
                          );
                        ...
                        if (json?.pdfSignedUrl) {
                          try {
                            ...
                            if (!r.ok)
                              throw new Error(
                                "No se pudo descargar PDF convertido",
                              );
                            ...
                            setConversionError(
                              "Error descargando PDF convertido",
                            );
                          }
                        }
                      } catch (err) {
                  ...
                  <Text
                    style={{ color: colors.text, marginTop: 8, fontSize: 12 }}
                  >
                    Se abrirá en el visor integrado en la app sin descargar el
                    archivo.
                  </Text>
                ...
                {conversionError && (
                  <Text style={{ color: "#F44336", marginTop: 8 }}>
                    {conversionError}
                  </Text>
                )}
              </>
    ...
    try {
      ...
      return (
        <Text style={{ color: colors.text }}>
          No se puede mostrar este archivo. Puedes descargarlo para verlo
          localmente.
        </Text>
      );
    }
      ...
      >
        ...
        >
          <Text style={{ color: theme.colors.textSecondary, fontSize: 13 }}>
            Puedes descargar el archivo desde las opciones del módulo.
          </Text>
        </View>
        ...
        {embeddedViewerUrl && (
          <Modal
            visible={!!embeddedViewerUrl}
            animationType="slide"
            onRequestClose={() => setEmbeddedViewerUrl(null)}
          >
            <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
              ...
              >
                <TouchableOpacity
                  onPress={() => setEmbeddedViewerUrl(null)}
                  style={styles.closeBtn}
                  accessibilityLabel="Cerrar"
                >
                  <Ionicons name="close" size={24} color={theme.colors.text} />
                  ...
                  (() => {
                    try {
                      ...
                      return (
                        <WebView
                          ...
                          onHttpError={(ev: any) => {
                            Alert.alert(
                              "Error",
                              "El visor integrado devolvió un error al cargar el archivo. Puedes solicitar conversión o descargar el archivo.",
                              [
                                {
                                  text: "Cerrar",
                                  style: "cancel",
                          ...
                          onError={(e: any) => {
                            Alert.alert(
                              "Error",
                              "No fue posible cargar el visor integrado. Puedes descargar el archivo o solicitar conversión.",
                            );
                          }}
                          style={{ flex: 1 }}
                        />
                      ...
                      return (
                        ...
                        >
                          <Text>
                            No fue posible abrir el visor integrado en este
                            dispositivo.
                          </Text>
                        </View>
                      );
