...
export const CourseFormModal: React.FC<Props> = ({
  ...
  const startInputIdRef = React.useRef(
    `mp_fecha_inicio_${Math.random().toString(36).slice(2, 9)}`,
  );
  const endInputIdRef = React.useRef(
    `mp_fecha_fin_${Math.random().toString(36).slice(2, 9)}`,
  );
      ...
      if (curso) {
        ...
        if (curso.id_instructor && !instructorName) {
          const inst = instructors.find(
            (i) => i.id === String(curso.id_instructor),
          );
          if (inst) instructorName = inst.name;
        }

        const durationHours = curso.duracion
          ? Math.round(curso.duracion / 60)
          : 1;

        let categoryName = curso.categoria || "";
        if (curso.id_categoria && categories.length > 0) {
          const cat = categories.find(
            (c) => String(c.id) === String(curso.id_categoria),
          );
  ...
  const instructorOwns = Boolean(
    courseId &&
    state.user &&
    form.id_instructor &&
    Number(form.id_instructor) === Number(state.user?.id_usuario),
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let finalInstructorId = form.id_instructor
        ? Number(form.id_instructor)
        : null;
      if (
        isInstructor &&
        courseId &&
      ...
      const attemptSave = async (pld: any) => {
        ...
        for (let i = 0; i < 5; i++) {
          try {
            ...
            const m =
              /Could find the '([^']+)' column of '([^']+)' in the schema cache|Could not find the '([^']+)' column of '([^']+)' in the schema cache/i.exec(
                e?.message || raw,
              );
        ...
        throw new Error(
          "Reintento fallido tras eliminar columnas desconocidas",
        );
      };
      ...
      if (
        /permission|policy|rls/i.test(raw) ||
        /not permitted/i.test(composed)
      ) {
        setFormError(
          `No tienes permisos para crear cursos desde el cliente. Detalles: ${composed}\n\nRAW: ${raw}`,
        );
      } else if (
        /constraint|violates/i.test(composed) ||
        /constraint/i.test(raw)
      ) {
        ...
        >
          ...
          <ScrollView style={styles.form}>
            ...
            <TextInput
              ...
              value={String(form.duracion_horas)}
              keyboardType="numeric"
              onChangeText={(t) =>
                setForm({ ...form, duracion_horas: Number(t) || 0 })
              }
            />
            <Text style={[styles.label, { color: theme.colors.text }]}>
              Categoría
            ...
            {showCategoryPicker && (
              ...
              >
                <ScrollView nestedScrollEnabled>
                  ...
                  {categories.length === 0 && (
                    <Text
                      style={{ padding: 8, color: theme.colors.textSecondary }}
                    >
            ...
            <TouchableOpacity
              style={[
                styles.input,
                {
                  ...
                  backgroundColor: theme.colors.card,
                  borderColor: theme.colors.border,
                  opacity: disableInstructorChange ? 0.6 : 1,
                },
              ]}
              onPress={() => {
                if (disableInstructorChange) {
                  Alert.alert(
                    "No permitido",
                    "No puedes cambiar el instructor de un curso que te pertenece.",
                  );
                  return;
                }
              ...
              accessibilityLabel={
                disableInstructorChange
                  ? "Instructor (no editable)"
                  : "Seleccionar instructor"
              }
              accessibilityHint={
                disableInstructorChange
                  ? "No puedes reasignar un curso que te pertenece"
                  : "Abre la lista de instructores"
              }
            >
              <Text style={{ color: theme.colors.text }}>
                {form.instructor || "Seleccionar instructor"}
              </Text>
            </TouchableOpacity>
            ...
            {disableInstructorChange && (
              ...
              >
                Este curso está asignado a ti; no puedes reasignarlo a otro
                instructor.
              </Text>
            )}
            <Text style={[styles.label, { color: theme.colors.text }]}>
            ...
            {Platform.OS === "web" ? (
              <div style={{ marginBottom: 8 }} className="mp-date-wrapper">
                ...
                />
                <label
                  htmlFor={startInputIdRef.current}
                  ...
                  onClick={(e: any) => {
                    try {
                      ...
                      } else if (startInputRef.current?.click) {
                ...
                >
                  ...
                  >
                    <path
                      d="M7 10H9V12H7V10ZM11 10H13V12H11V10ZM15 10H17V12H15V10Z"
                      fill="currentColor"
                    />
                  </svg>
                </label>
                ...
                >
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text
                      style={{
                        marginLeft: 8,
                        color: form.fecha_inicio
                          ? theme.colors.text
                          : theme.colors.textSecondary,
                      }}
                    >
                      {form.fecha_inicio || "Seleccionar fecha"}
                    </Text>
                  </View>
                </TouchableOpacity>
                {showStartPicker &&
                ...
                <label
                  ...
                  onClick={(e: any) => {
                    try {
                      ...
                      } else if (endInputRef.current?.click) {
                ...
                >
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text
                      style={{
                        marginLeft: 8,
                        color: form.fecha_fin
                          ? theme.colors.text
                          : theme.colors.textSecondary,
                      }}
                    >
                      {form.fecha_fin || "Seleccionar fecha"}
                    </Text>
                  </View>
                </TouchableOpacity>
                {showEndPicker &&
